...
Veloro uses one shared Neon project for both the storefront and admin applications.

We are **not** using Neon's one-click Vercel preview integration as the primary setup right now.

Reason:

- Veloro has two separate Vercel projects: `snn-storefront` and `snn-admin`
...
Use the default primary branch as production for now.
...
- Database: `veloro`
- Runtime role: `veloro_app`
- Migration role: `veloro_migrator`
...
```sql
CREATE ROLE veloro_app LOGIN PASSWORD 'replace-with-generated-password';
CREATE ROLE veloro_migrator LOGIN PASSWORD 'replace-with-generated-password';

CREATE DATABASE veloro;

GRANT CONNECT ON DATABASE veloro TO veloro_app;
GRANT CONNECT ON DATABASE veloro TO veloro_migrator;
```

After connecting to the `veloro` database as the project owner or admin role:

```sql
GRANT USAGE, CREATE ON SCHEMA public TO veloro_migrator;
GRANT USAGE ON SCHEMA public TO veloro_app;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO veloro_app;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO veloro_app;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT ALL ON TABLES TO veloro_migrator;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT ALL ON SEQUENCES TO veloro_migrator;
```

For the foundation phase, using `veloro_migrator` for both runtime and migrations is acceptable if you want the simpler path first. The more secure target state is separate runtime and migration roles.
...
That will be more correct for Veloro than using a one-to-one marketplace integration on only one of the apps.
